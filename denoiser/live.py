...
def get_parser():
    ...
    parser.add_argument(
        "--device", default="cpu", help="Device to use for processing.")
    parser.add_argument(
    ...
    parser.add_argument(
        "-t", "--num_threads", type=int, default=4,
    ...
    parser.add_argument(
        "--output_file", type=str, default="output.wav",
...
def main():
    ...
    try:
        ...
        print("Stopping")
        processed_audio = np.concatenate(processed_frames, axis=0).astype(np.float32).tobytes()
        save_wav(args.output_file, processed_audio, model.sample_rate)
